...
export const useNoteFormatter = ({ editorRef, updateActiveNote }: UseNoteFormatterProps) => {
  ...
  const handleInsertCallout = useCallback(
    (type: 'tip' | 'warning' | 'question' | 'code') => {
      ensureEditorFocus();
      const templates = {
        tip: '<div style="background:#fef3c7;border-inline-start:4px solid #f59e0b;padding:12px;border-radius:10px;margin:12px 0;color:#78350f"><strong>💡 فكرة / نصيحة:</strong> اكتب فكرتك أو ملحوظتك هنا...</div><p><br></p>',
        warning: '<div style="background:#ffe4e6;border-inline-start:4px solid #f43f5e;padding:12px;border-radius:10px;margin:12px 0;color:#881337"><strong>⚠️ تنبيه برمجي:</strong> انتبه لهذا الخطأ الشائع...</div><p><br></p>',
        question: '<div style="background:#e0f2fe;border-inline-start:4px solid #0284c7;padding:12px;border-radius:10px;margin:12px 0;color:#075985"><strong>❓ سؤال للمراجعة في الزووم:</strong> استفسار لمناقشته مع المينتور...</div><p><br></p>',
