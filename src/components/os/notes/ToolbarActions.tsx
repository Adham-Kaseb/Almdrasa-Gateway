  ...
  return (
    <div className="flex items-center gap-1">
      <button
        ...
        title={isLtr ? "Copy note content" : "نسخ محتوى الملاحظة"}
      >
        {isCopied ? <Check className="w-3.5 h-3.5 text-emerald-600" /> : <Copy className="w-3.5 h-3.5" />}
        <span className="hidden sm:inline">
          {isCopied ? (isLtr ? 'Copied!' : 'تم النسخ!') : (isLtr ? 'Copy' : 'نسخ')}
        </span>
      </button>
      <button
        type="button"
        ...
        onClick={onExport}
        className="p-2 rounded-xl bg-white/70 hover:bg-white border border-[#E5E0D6] text-[#3C3831] hover:text-[#191816] text-xs font-bold flex items-center gap-1.5 transition-all cursor-pointer shadow-2xs active:scale-95"
        title={isLtr ? "Export as text file" : "تصدير كملف نصي"}
      >
        <Download className="w-3.5 h-3.5" />
        <span className="hidden sm:inline">{isLtr ? 'Export' : 'تصدير'}</span>
      </button>
      <button
        type="button"
        onMouseDown={(e) => e.preventDefault()}
        ...
        title={isLtr ? "Start blank page" : "بدء صفحة بيضاء جديدة فارغة"}
      >
        <RotateCcw className="w-3.5 h-3.5" />
        <span className="hidden sm:inline">{isLtr ? 'Blank Page' : 'صفحة بيضاء'}</span>
      </button>
    </div>
  );
};
